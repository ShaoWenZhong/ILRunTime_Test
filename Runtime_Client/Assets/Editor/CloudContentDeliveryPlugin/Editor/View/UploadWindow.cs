...
namespace CloudContentDelivery
{
    public class UploadWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            // EditorGUI.LabelField(new Rect(3, 3, position.width - 6, 20), "Total Files : ", Parameters.totalUploadFiles.ToString());
            // EditorGUI.LabelField(new Rect(3, 25, position.width - 6, 20), "Total Size : ", Parameters.totalUploadSize.ToString());
